...
function AwesomeIconPicker({ value, onChange }) {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-12 z-50 w-80 rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
          <div className="mb-2 flex flex-wrap gap-1">
            {AWESOME_GROUPS.map((g, i) => (
              <button
                key={g.label}
                type="button"
                onClick={() => setGroup(i)}
          ...
          <div className="grid grid-cols-10 gap-1">
            {AWESOME_GROUPS[group].icons.map((icon) => (
              <button
                key={icon}
                type="button"
