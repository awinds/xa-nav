...
export async function onRequestPut({ request, env, params }) {
  ...
  const id = params.id;
  const cat = await env.db.prepare('SELECT is_default FROM categories WHERE id = ?').bind(id).first();
  ...
  await env.db.prepare('UPDATE categories SET name=?, parent_id=?, icon=?, sort_order=?, is_private=? WHERE id=?')
